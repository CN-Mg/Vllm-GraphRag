...
/* FileTable 组件:文件管理的核心 UI
负责展示上传的文件列表及其进度，提供文件查看、取消处理等功能 */
// 集成 API 调用：轮询文件处理状态（PollingAPI）、取消处理（CancelAPI）、更新服务端状态（ServerSideStatusUpdateAPI）
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor((row) => row, {
        id: 'source',
        cell: (info) => {
          if (info.row.original.fileSource === 'web-url') {
            return (
              <Flex>
                <span>
