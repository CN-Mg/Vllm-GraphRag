...
const Chatbot: React.FC<ChatbotProps> = (props) => {
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    // image+graph+vector 模式：显示图片选择对话框
    if (chatMode === 'image+graph+vector') {
    ...
    try {
      ...
      const chatbotAPI = await chatBotAPI(
        ...
        chatMode === 'image+graph+vector' ? selectedImageUrl : undefined
      );
  ...
  const handleImageAnalysis = async (imageUrl: string, userQuestion: string) => {
    setSelectedImageUrl(imageUrl);
    const datetime = `${date.toLocaleDateString()} ${date.toLocaleTimeString()}`;
    const userMessage = { id: Date.now(), user: 'user', message: userQuestion, datetime: datetime };
    setListMessages([...listMessages, userMessage]);
    ...
    try {
      ...
      const chatresponse = chatbotAPI?.response;
      const chatbotReply = chatresponse?.data?.data?.message;
      const chatSources = chatresponse?.data?.data?.info.sources;
      const chatModel = chatresponse?.data?.data?.info.model;
      const chatChunks = chatresponse?.data?.data?.info.chunkdetails;
      const chatTokensUsed = chatresponse?.data?.data?.info.total_tokens;
      const chatTimeTaken = chatresponse?.data?.data?.info.response_time;
      const chatingMode = chatresponse?.data?.data?.info.mode;
      const cypher_query = chatresponse?.data?.data?.info.cypher_query ?? '';
      const graphonly_entities = chatresponse?.data?.data?.info.graph_context ?? [];
      ...
      const chatbotReply = "Oops! It seems we couldn't analyze the image. Please try again later";
      setInputMessage('');
      simulateTypingEffect({ reply: chatbotReply });
    }
  };
  ...
  return (
    <div className='n-bg-palette-neutral-bg-weak flex flex-col justify-between h-full overflow-hidden'>
      <div className='flex overflow-y-auto pb-12 w-full flex-1 chatBotContainer pl-3 pr-3'>
      ...
      {/* Image Selection Modal for image+graph+vector mode */}
      <ImageSelectionModal
        open={showImageModal}
        onClose={() => {
          setShowImageModal(false);
        ...
        onImageSelected={(imageUrl) => setSelectedImageUrl(imageUrl)}
        onSendMessage={handleImageAnalysis}
      />
    </div>
  );
};
