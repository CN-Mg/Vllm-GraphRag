...
from src.image_analysis import get_image_nodes_with_chunks, analyze_image_with_vlm, get_image_chunk_context
import uvicorn
import asyncio
import base64
...
async def get_source_list(uri:str, userName:str, password:str, database:str=None):
    """
    调用"get_source_list_from_graph",该函数会返回已存在于数据库中的数据源列表,以供前端展示和选择.
    """
    try:
...
async def post_processing(uri=Form(None), userName=Form(None), password=Form(None), database=Form(None), tasks=Form(None)):
    """ 
    /extract API 完成后，前端调用/post_processing做收尾处理
    生成KNN相似分块关系(基于Embedding向量,阈值KNN_MIN_SCORE=0.8)
    + 删除并重建Neo4j全文索引
    """
    try:
        graph = create_graph_database_connection(uri, userName, password, database)
...
#@app.get("/images_list")
@app.api_route("/images_list", methods=["GET", "POST"]) # 支持GET和POST方法，以适应不同前端调用方式
async def get_images_list(uri=Form(None), userName=Form(None), password=Form(None), database=Form(None), document_names=Form(None)):
    """
    获取图片列表及其关联的chunks
    ...
    try:
        graph = create_graph_database_connection(uri, userName, password, database)
        from src.image_analysis import get_image_nodes_with_chunks

        # 解析document_names
        doc_names = None
        if document_names:
            doc_names = list(map(str.strip, json.loads(document_names)))

        images = await asyncio.to_thread(get_image_nodes_with_chunks, graph, doc_names)
        ...
        logger.log_struct(josn_obj)
        return create_api_response('Success', data=images, message=f'Retrieved {len(images)} images')

    except Exception as e:
        job_status = "Failed"
        message = "Unable to get images list"
        error_message = str(e)
        logging.exception(f'Exception in get images list: {error_message}')
        return create_api_response(job_status, message=message, error=error_message)
    finally:
        gc.collect()
        if graph is not None:
            close_db_connection(graph, 'images_list')

@app.post("/chat_bot")
async def chat_bot(uri=Form(None),model=Form(None),userName=Form(None), password=Form(None), database=Form(None),question=Form(None), document_names=Form(None),session_id=Form(None),mode=Form(None),image_url=Form(None)):
    logging.info(f"QA_RAG called at {datetime.now()} with mode: {mode}")
    ...
    try:
        if mode == "graph":
            ...
            graph = create_graph_database_connection(uri, userName, password, database)
        result = await asyncio.to_thread(QA_RAG,graph=graph,model=model,question=question,document_names=document_names,session_id=session_id,mode=mode,image_url=image_url)
...
def generate_text(chat_request: Annotated[ChatRequest, Body()])->Dict:
    ...
    response = client.chat.completions.create(
        ...
        messages=[m.model_dump() for m in chat_request.messages],  # 数据结构转换 model_dump is used to convert Pydantic model to dict
