...
from src.image_analysis import (
    get_image_nodes_with_chunks,
    analyze_image_with_vlm,
    get_image_chunk_context
)
from src.llm import get_llm
...
def QA_RAG(graph, model, question, document_names, session_id, mode, image_url=None):
    try:
        ...
        if mode == "image+graph+vector":
            ...
            ai_response = AIMessage(content=analysis_result["response"])
            messages.append(ai_response)
            summarize_and_log(history, messages, get_llm(VISION_LLM_MODEL)[0])
