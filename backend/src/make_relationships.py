...
def create_relation_between_chunks(graph, file_name, chunks: List[Document])->list:
    ...
    for i, chunk in enumerate(chunks):
        ...
        if 'images' in chunk.metadata and chunk.metadata['images']:
            image_urls = [img.get('img_url', '') for img in chunk.metadata['images'] if img.get('img_url')]
            if image_urls:
                chunk_data['image_urls'] = image_urls
    ...
    query_to_create_chunk_and_PART_OF_relation = """
        ...
        SET c.page_number = CASE WHEN data.page_number IS NOT NULL THEN data.page_number END,
            ...
            c.imageUrls = CASE WHEN data.image_urls IS NOT NULL THEN data.image_urls END
        WITH data, c
...
def create_image_nodes_and_relationships(graph: Neo4jGraph, file_name: str, all_image_info_list: List[Dict[str, Any]]):
    """
    创建 Image 节点并建立与 Document 的关系
    ...
    """
    if not all_image_info_list:
        logging.info("No images to process")
        return

    logging.info(f"Creating {len(all_image_info_list)} image nodes for file: {file_name}")
    ...
    for img_info in all_image_info_list:
        batch_data.append({
            ...
            "img_index": img_info.get("img_index"),
            "img_format": img_info.get("img_format", "png"),
    ...
    """
    graph.query(query_to_create_image_nodes, params={"batch_data": batch_data})

    logging.info(f"Successfully created {len(batch_data)} image nodes")


def create_chunk_image_relationships(graph: Neo4jGraph, file_name: str, chunks_with_images: List[Dict[str, Any]]):
    """
    创建 Chunk 与 Image 之间的关系

    Args:
        graph: Neo4j 图数据库连接
        file_name: 文件名
        chunks_with_images: 包含图片信息的 chunk 列表
    """
    if not chunks_with_images:
        return

    relationship_data = []
    for chunk_info in chunks_with_images:
        chunk_id = chunk_info.get("chunk_id")
        images = chunk_info.get("images", [])
    ...
    logging.info(f"Creating {len(relationship_data)} chunk-image relationships")

    query_to_create_relationships = """
        UNWIND $relationship_data AS data
        MATCH (c:Chunk {id: data.chunk_id})
        MATCH (i:Image {id: data.img_id})
        MERGE (c)-[:CONTAINS_IMAGE]->(i)
    """
    graph.query(query_to_create_relationships, params={"relationship_data": relationship_data})

    logging.info(f"Successfully created chunk-image relationships")